...
def render_html_from_data(data: dict) -> str:
    """
    Given the AICO JSON data, return a full HTML string
    for the Product Facts page.
    """
    meta = data.get("meta", {})
...
def write_html_file(slug: str, html: str) -> Path:
    """
    Write the rendered HTML to /products/<slug>.html.
    Example: slug 'noco-gb40' -> products/noco-gb40.html
    """
    out_path = REPO_ROOT / "products" / f"{slug}.html"
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python engine/render_product.py <slug>")
        print("Example: python engine/render_product.py noco-gb40")
        sys.exit(1)

    slug = sys.argv[1]
    data = load_product_json(slug)
    html = render_html_from_data(data)
